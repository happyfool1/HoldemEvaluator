...
/*- ******************************************************************************
* This class contains all of the Preflop ranges for one player type.
* Hero, HeroX, Average Averagex, Reg, Lag, Tag, Fish, Looser, and whatever else
* the future may hold. 
 ...
 * 
 * In previous versions a 13 X 13 array was indexed directly.
 ...
 * 
 * The purpose of this class ( and the previous HandRangeCumulative class) is to select an action
 ...
 * Ranges are created in either playability or EV order, so it is contiguous with
 ...
 * The first index is the position, SB, BB, UTG, MP, CO, BU
 * The second index is the action to the players, PREFLOP_x. 
...
public class HandRangeCumulative implements Constants, java.io.Serializable {
	...
	* Find hand in drawAndMadeArray and return index Arg0 - hand in string format.
	...
	int getHandStringToIndex(String hand) {
		...
		Logger.log("ERROR HandRangeCumulative handStringToIndex() Invalid hand " + hand);
		return -1;
	}
	...
	*******************************************************************************/
	void show() {
		constructPanel(" HandRangeCumulative ");
	}
	...
	*  Construct panel. 
	 ...
	 * Arg0 - The full path name.
	  *****************************************************************************/
	HandRangeCumulative readFromFile(String path) {
		HandRangeCumulative r = null;

		try (var fileIn = new FileInputStream(path); var in = new ObjectInputStream(fileIn)) {
			r = (HandRangeCumulative) in.readObject();
		} catch (IOException i) {
			i.printStackTrace();
		...
		if (r == null || r.rangeArrayRaise[2][0] == null) {
			Logger.logError("Error readFromFile() handRangeCumulative = null");
		}
		return r;
	}
